...
public class HdfsTest {
            ...
            for (int j = 0; j <= 9; j++) {
                for (int i = 1; i <= 2; i++) {
                    File syslogFile = new File(
                            queueDirectory.toUri() + File.separator + "testConsumerTopic" + j + "." + i
                    );
                    ...
                    HDFSWrite writer = new HDFSWrite(config, lastObject, lastObjectJo);
    ...
    public void hdfsReadCheck(String testConsumerTopic, int partition, long offset) {
